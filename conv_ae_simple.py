...
def DigitRecogModel(dir_train, phase='training', model_path_load=None, model_path_save=r'./model_simple.pt'):
    ...
    # setting parameter
    batch_size = 5
    epoch_loop = 5000 if phase == 'training' else 1
    ...
    for epoch in range(epoch_loop):
        ...
        if (epoch + 1 ) % 10 == 0 and  model_path_save != None:
            torch.save({
...
def getValueFromJson(file_json, model_path_load=r'./model_simple_autoen_7segment.pt'):
    global is_cuda, optimizer, train_transform
    is_cuda = False
    if torch.cuda.is_available():
        is_cuda = True
...
if __name__ == '__main__':
    ...
    # 처음 학습시킬때.
    loss, acc = DigitRecogModel(r'.\digit_class_aug_autoencoder', phase='training', model_path_load=None,
                                model_path_save=r'./model_simple_autoen_7segment.pt')
